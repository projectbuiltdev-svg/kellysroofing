      ...
      <header className="absolute inset-x-0 top-0 z-40">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
          <a href="#top" onClick={closeMenu} className="group flex items-center gap-3" data-testid="link-logo">
            <span className="flex h-[52px] w-[148px] items-center justify-center overflow-hidden rounded bg-[#fbfaf6] p-1 shadow-lg shadow-[#0c1b31]/10">
              <img src={logoPath} alt="Kellys Roofing and Interiors" className="h-full w-full object-contain" />
            </span>
          </a>
        ...
        <div className="mx-auto max-w-[1380px]">
          <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
            <div className="max-w-[720px]">
              ...
              <h2 className="display-title mt-5 text-[clamp(3rem,6vw,5.7rem)] font-semibold text-[#10233f]">The work behind<br /><span className="text-[#c75632]">a better building.</span></h2>
            </div>
              ...
              return (
                <article key={service.number} className="service-card rounded-lg border border-[#c9c5bb] bg-[#f8f6f0] p-6 sm:p-8" data-testid={`card-service-${service.number}`}>
                  <div className="flex items-start justify-between">
                    <span className="mono text-[11px] font-bold text-[#c75632]">{service.number}</span>
                  ...
                  <button onClick={() => setOpenService(isOpen ? null : service.number)} className="mt-7 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.12em] text-[#10233f]" aria-expanded={isOpen} data-testid={`button-service-details-${service.number}`}>
                    {isOpen ? 'Close details' : 'See what is involved'} {isOpen ? <Minus size={15} className="text-[#c75632]" /> : <MoveUpRight size={15} className="text-[#c75632]" />}
                  </button>
                </article>
              );
            })}
      ...
      <section id="approach" className="bg-[#dbe3e4] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
        <div className="mx-auto grid max-w-[1380px] gap-14 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
          <div>
            ...
            <h2 className="display-title mt-5 max-w-[450px] text-[clamp(3rem,6vw,5.3rem)] font-semibold text-[#10233f]">No fog.<br />Just a <span className="text-[#c75632]">clear route</span> through the work.</h2>
          ...
          <div className="divide-y divide-[#aab9bc] border-y border-[#aab9bc]">
            {projectNotes.map(([number, title, text]) => (
              <div key={number} className="grid grid-cols-[50px_1fr] gap-5 py-8 sm:grid-cols-[80px_1fr] sm:gap-8">
                <span className="mono text-[11px] font-bold text-[#c75632]">{number}</span>
                <div>
       ...
       <section id="about" className="page-grid bg-[#f4f0e7] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
        <div className="mx-auto grid max-w-[1380px] items-center gap-12 lg:grid-cols-[1.1fr_.9fr] lg:gap-24">
          ...
          <div>
            ...
            <h2 className="display-title mt-5 text-[clamp(2.8rem,5vw,4.7rem)] font-semibold text-[#10233f]">Built around<br /><span className="text-[#c75632]">your peace of mind.</span></h2>
            ...
            <button onClick={scrollToContact} className="group mt-8 inline-flex items-center gap-3 rounded-full bg-[#10233f] px-5 py-3 text-sm font-bold text-[#fbfaf6] transition hover:bg-[#c75632]" data-testid="button-about-contact">
